    ...
    columns.forEach(col => {
        ...
        if (col.isCode) {
            ...
            for (let j = 0; j < text.length; j++) {
                ...
                const alpha = j === text.length - 1 ? 0.9 : (1 - headDist) * col.brightness * 0.5;
                if (j === text.length - 1) {
            ...
            for (let j = 0; j < col.chars.length; j++) {
                ...
                if (j === col.chars.length - 1) {
                    ...
                    const alpha = (1 - headDist) * col.brightness * 0.4;
                    ctx.fillStyle = `rgba(0, 255, 157, ${alpha})`;
...
function drawNeuralNetwork(time) {
    ...
    for (let i = pulses.length - 1; i >= 0; i--) {
        ...
        grd.addColorStop(0, `rgba(${p.color.join(',')}, 0.6)`);
        grd.addColorStop(0.3, `rgba(${p.color.join(',')}, 0.2)`);
    ...
    nodes.forEach(node => {
        ...
        ctx.strokeStyle = `rgba(0, 255, 157, ${0.45 * node.activity * pulse})`;
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ...
        ctx.fillStyle = `rgba(0, 255, 157, ${0.6 * node.activity})`;
        ctx.fill();
    });
}
